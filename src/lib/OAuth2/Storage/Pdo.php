    ...
    PublicKeyInterface
{
    ...
    public function __construct($connection, $config = array())
    {
        ...
        $this->config = array_merge(array(
            'client_table' => 'test_oauth_clients',
            'access_token_table' => 'test_oauth_access_tokens',
            'refresh_token_table' => 'test_oauth_refresh_tokens',
            'code_table' => 'test_oauth_authorization_codes',
            'user_table' => 'test_oauth_users',
            'jwt_table'  => 'test_oauth_jwt',
            'scope_table'  => 'test_oauth_scopes',
            'public_key_table'  => 'test_oauth_public_keys',
        ), $config);
    }
    ...
    {
        $stmt = $this->db->prepare($sql = sprintf('SELECT scope FROM %s WHERE (client_id=:client_id OR client_id IS NULL) ORDER BY client_id IS NOT NULL DESC', $this->config['scope_table']));
